...
extern TaskHandle_t SPI_Task_Handler;
...
void PORTA_IRQHandler() {
	...
	if (PTA1 == (InterruptFlags & PTA1)) {
		xEventGroupSetBitsFromISR(UpdateValueByButtons_Events, MONTH_DOWN,
				&pxHigherPriorityTaskWoken);
		GPIO_ClearPinsInterruptFlags(GPIOA, PTA1);
	} else if (PTA2 == (InterruptFlags & PTA2)) {
		xEventGroupSetBitsFromISR(UpdateValueByButtons_Events, DAY_DOWN,
				&pxHigherPriorityTaskWoken);
...
void PORTB_IRQHandler() {
	...
	if (PTB9 == (InterruptFlags & PTB9)) {
		xEventGroupSetBitsFromISR(UpdateValueByButtons_Events, MONTH_UP,
				&pxHigherPriorityTaskWoken);
		GPIO_ClearPinsInterruptFlags(GPIOB, PTB9);

	} else if (PTB23 == (InterruptFlags & PTB23)) {
		xEventGroupSetBitsFromISR(UpdateValueByButtons_Events, DAY_UP,
				&pxHigherPriorityTaskWoken);
...
static TerminalMenuType_t Menus[MENUS_QUANTITY] = { //menus strings to be displayed initialization
		/**0*/{ 9, { { "1) Leer Memoria I2C", "\033[3;10H" }, {
				"2) Escribir memoria I2C", "\033[4;10H" }, {
				"3) Establecer Hora", "\033[5;10H" }, { "4) Establecer Fecha",
				"\033[6;10H" }, { "5) Formato de hora", "\033[7;10H" }, {
				"6) Leer hora", "\033[8;10H" },
				{ "7) Leer fecha", "\033[9;10H" }, {
		...
		/**3*/{ 3, { { "3) Establecer Hora", "\033[5;10H" }, {
				"Escribir hora en hh:mm:ss: ", "\033[7;10H" }, {
				""/*"7:30:55"*/, "\033[7;37H" } } },
		/**4*/{ 3, { { "4) Establecer Fecha", "\033[6;10H" }, {
				"Escribir fecha en dd/mm/aa: ", "\033[8;10H" }, {
				""/*"24:12:2017"*/, "\033[8;38H" } } },
		/**5*/{ 5, { { "5) Formato de hora", "\033[7;10H" }, {
				"El formato actual es ", "\033[9;10H" }, { ""/*"12h"*/,
				"\033[9;31H" }, { "Escoger formato 12 o 24 (1 / 0): ",
				"\033[10;10H" }, { ""/*"s"*/, "\033[10;43H" } } },
		...
		/**8*/{ 5/*variable*/, { { "8) Comunicacion con terminal 2",
				...
				"\033[15;10H" },
				{ ""/*Bien, aqui sufriendo con la practica y tu?*/,
						"\033[16;10H" } } },
		/**9*/{ 3, { { "9) Eco en LCD", "\033[11;10H" }, { "Escribir texto: ",
				"\033[13;10H" }, { ""/*"texto"*/, "\033[14;10H" } } },
		/**10*/{ 6,
				{ { "ERROR", "\033[3;28H" }, {
						"Ocasionado por uno de los siguientes motivos: ",
						"\033[5;08H" },
						{ "I) Componente faltante", "\033[7;07H" },
						{ "II) Menu de componente ocupado por otra terminal",
								"\033[8;07H" }, {
								"III) Error en la interaccion con un menu",
								"\033[9;07H" }, {
								"Presione 2 veces = para continuar...",
								"\033[12;08H" } } } };
...
void MenuPrinter(uart_struct* uart_struct, uint8_t menuToBePrinted) {
	...
	uint8_t printedStringIndex = 0; /** Index used to know wich string has been printed*/
	...
	while (Menus[menuToBePrinted].Strings_quantity > printedStringIndex) /**While there is still any string to be printed then,*/
	{
		toSend_UART = pvPortMalloc(sizeof(uart_transfer_t)); /**Memory reservation for the uart send queue*/

		/** the data to be sent will be the terminal position VT100 command to print the string*/
		toSend_UART->data =
		...
		/** the data to be printed will be the strings that compund the corresponding menu*/
		toSend_UART->data =
...
void CreateMenuTask(uart_struct* uart_struct, uint8_t menuToBeCreated) {
	...
	case ReadMemoryI2C:
		...
		configMINIMAL_STACK_SIZE + 20, (void*) uart_struct, 2, NULL);

		break;
	case WriteMemoryI2C:
		xTaskCreate(TerminalMenus_WriteMemory, "Write memory menu",
		configMINIMAL_STACK_SIZE + 50, (void*) uart_struct, 2, NULL);
	...
	case ReadDate:
		...
		configMINIMAL_STACK_SIZE + 10, (void*) uart_struct, 2, NULL);
		break;
	case Terminal2Communication:
	...
	case LCDEcho:
		...
		configMINIMAL_STACK_SIZE + 25, (void*) uart_struct, 2, NULL);
		break;
	default:
		break;
	}
...
void CheckIfReturnToMenu(uart_struct* uart_struct, uint8_t receivedChar,
	...
	if (RETURN == receivedChar) {
		...
		configMINIMAL_STACK_SIZE, (void*) uart_struct, 3, NULL); //create menu task
		switch (type) {
		case MEMORY_TASK:
		...
		case RTC_TASK:
			...
			break;
		case SPI_TASK:
			xEventGroupSetBits(SubTasks_Events, LCD_FREE);
			break;
		}
...
void CheckIfLineMovementAndUartEcho(uart_struct* UART_struct,
		uint8_t receivedChar, LineMovementPack_t * lineMoverPack) {
...
void TerminalMenus_MainMenu(void* args) {
	...
	vTaskDelay(pdMS_TO_TICKS(500)); /**gives a little bit of time by sleeping the task in order to have everuthing properly configured*/
	for (;;) { /**task superloop*/
		if (pdFALSE == firstEntryToMenu) {/**if its the first time that the task enters the superloop then,*/
			...
			MenuPrinter(UART_struct, MainMenu);
			xSemaphoreGive(Interface_mutex); /**releases the previously taken mutex*/
		}
		xQueueReceive(*UART_struct->UART_receive_Queue, &received_UART, /**sleeps until a char is received from the uart*/
		portMAX_DELAY);
		charReceived = *received_UART->data; /**the received char is stored*/

		if (('1' <= charReceived) && ('9' >= charReceived)) {
...
void TerminalMenus_ReadRTCHour(void* args) {
	...
	for (;;) {
		...
		if (pdPASS
...
#if 1
			uint8_t EventValues = xEventGroupGetBits(
					UpdateValueByButtons_Events);
			if (0 != EventValues) {

				uint8_t hours = ((time_buffer_variable[2] >> 4) & 0x03) * 10
						+ (time_buffer_variable[2] & 0xF);
				uint8_t minutes = (time_buffer_variable[1] >> 4) * 10
						+ (time_buffer_variable[1] & 0xF);
				...
				time_buffer_variable[2] = ((time_buffer_variable[2]
						& TimeFormatMaskSet)
						| (((hours / 10) << 4) + (hours - (hours / 10) * 10)));
				time_buffer_variable[1] = ((minutes / 10) << 4)
						+ ((minutes - (minutes / 10) * 10));

				i2c_master_transfer_t* masterXfer_changed_hours;
				masterXfer_changed_hours = pvPortMalloc(
						sizeof(i2c_master_transfer_t));
				...
				masterXfer_changed_hours->subaddress =
						(uint32_t) minutes_subaddres;
				masterXfer_changed_hours->subaddressSize = 1;

				vTaskDelay(pdMS_TO_TICKS(50));
				xQueueSend(*pI2C_write_queue, &masterXfer_changed_hours,
						portMAX_DELAY);
				xSemaphoreTake(*pI2C_done, portMAX_DELAY);
				xSemaphoreGive(*pI2C_done);
			...
			xEventGroupClearBits(UpdateValueByButtons_Events,
					HOURS_UP | HOURS_DOWN | MINUTES_UP | MINUTES_DOWN);
#endif
...
//                xTimerStart(I2C_Timer_Handler,0);
				xQueueSend(*pI2C_write_queue, &masterXfer_I2C_write,
						portMAX_DELAY);
				xSemaphoreTake(*pI2C_done, portMAX_DELAY);
//                if(? == xTimerStop(I2C_Timer_Handler,0)){
//                    PRINTF(" ERROR DE LECTURA RTC");
//                }
			...
			if (0 != EventValues) {
				...
				uint8_t day = ((time_buffer_variable[0] >> 4) & 0x03) * 10
						+ (time_buffer_variable[0] & 0xF);
				uint8_t month = ((time_buffer_variable[1] >> 4) & 0x03) * 10
						+ (time_buffer_variable[1] & 0xF);
				...
				time_buffer_variable[1] = ((month / 10) << 4)
						+ ((month - (month / 10) * 10));
				time_buffer_variable[0] = ((time_buffer_variable[0] & 0xC0)
						| (((day / 10) << 4) + ((day - (day / 10) * 10))));

				i2c_master_transfer_t* masterXfer_changed_hours;
				masterXfer_changed_hours = pvPortMalloc(
						sizeof(i2c_master_transfer_t));
				uint32_t minutes_subaddres = 0x05;
				masterXfer_changed_hours->data = &(time_buffer_variable[0]);
				masterXfer_changed_hours->dataSize = 1;
				masterXfer_changed_hours->direction = kI2C_Write;
				masterXfer_changed_hours->flags = kI2C_TransferDefaultFlag;
				masterXfer_changed_hours->slaveAddress = 0x50;
				...
				masterXfer_changed_hours = pvPortMalloc(
						sizeof(i2c_master_transfer_t));
				uint32_t hours_subaddress = 0x06;
				masterXfer_changed_hours->data = &(time_buffer_variable[1]);
				masterXfer_changed_hours->dataSize = 1;
				masterXfer_changed_hours->direction = kI2C_Write;
				masterXfer_changed_hours->flags = kI2C_TransferDefaultFlag;
			...
			xEventGroupClearBits(UpdateValueByButtons_Events,
					DAY_UP | DAY_DOWN | MONTH_UP | MONTH_DOWN);
#endif
...
void TerminalMenus_TerminalsCommunication(void* args) {
	...
	for (;;) /**task superloop*/
	{
		...
		if (pdFALSE
				== UART_struct->ChatStates->FirstEntry[UART_struct->uart_number]) { /**if it's not the first time this particular terminal is inside this task then,*/
			...
			if ((UART_0 == UART_struct->uart_number)
					&& pdTRUE
							== (UART_struct->ChatStates->terminalChatStates[UART_1])) { /**if this is the terminal 0, and the other terminal is also inside the chat menu then,*/
				toSend_UART = pvPortMalloc(sizeof(uart_transfer_t)); /**memory is reserved for a UART transference*/
				...
				vTaskDelay(pdMS_TO_TICKS(20)); /**gives time for the UART to properly print the string sent*/
					...
					&& pdTRUE
							== (UART_struct->ChatStates->terminalChatStates[UART_0])) { /**if this is the terminal 1, and the other terminal is also inside the chat menu then,*/
				toSend_UART = pvPortMalloc(sizeof(uart_transfer_t)); /**memory is reserved for a UART transference*/
				toSend_UART->data = connection_array; /**the data to be sent will be the terminal identification string*/
			...
			if (pdTRUE == (UART_struct->ChatStates->terminalChatStates[UART_0])
					&& pdTRUE
							== (UART_struct->ChatStates->terminalChatStates[UART_1])) { /**if both terminals are in the chat menu*/
		...
		if (pdTRUE == (UART_struct->ChatStates->terminalChatStates[UART_0])
			...
			if (pdTRUE == receivedFromUart) { /**if something was received in the UART queue*/
				...
				if ((RETURN != charReceived[0]) && (ENTER != charReceived[0])) { /**if the received char is neither enter nor return*/
				...
				if (RETURN == charReceived[0]) { /**if the received char is a return to main menu,*/
					...
					if ((UART_1 == UART_struct->uart_number)
							&& pdTRUE
									== (UART_struct->ChatStates->terminalChatStates[UART_0])) { /**if the current terminal to exit is uart 1 and the other terminal is inside the chat menu,*/
						...
						vTaskDelay(pdMS_TO_TICKS(20)); /**gives time for the UART to properly print the string sent*/
							...
							&& pdTRUE
									== (UART_struct->ChatStates->terminalChatStates[UART_1])) { /**if the current terminal to exit is uart 1 and the other terminal is inside the chat menu,*/
				...
				if (ENTER == charReceived[0]) { /**if the received char was an enter (i.e. sent to the chat),*/
					toSend_UART = pvPortMalloc(sizeof(uart_transfer_t)); /**memory is reserved for a UART transference*/
					toSend_UART->data = UART_struct->ChatStates->chatPosition; /**the data to be sent through the UART is the shared terminal position*/
					xQueueSend(*UART_struct->ChatStates->UART0_send_Queue,
							&toSend_UART, portMAX_DELAY); /**data is sent to the terminal 1*/
					...
					while (pdTRUE != FIFO_checkIfEmpty(UART_struct->uart_number)) { /**while there is something to take out from the fifo,*/
					...
					for (index = 0; index < 51; index++) { /**loop that clears space by space the upmost space*/
						toSend_UART = pvPortMalloc(sizeof(uart_transfer_t)); /**memory is reserved for a UART transference*/
				...
				== UART_struct->ChatStates->terminalChatStates[UART_struct->uart_number]) { /**if only one terminal is inside the terminal chat*/
			if (pdTRUE == receivedFromUart) { /**if something was actually received from the UART Queue*/
...
void TerminalMenus_LCDEcho(void* args) {
	...
	xEventGroupWaitBits(SubTasks_Events, LCD_FREE, pdFALSE, pdTRUE,
			portMAX_DELAY);
	vTaskSuspend(SPI_Task_Handler);
	for (;;) { /**practic superloop*/
		xEventGroupWaitBits(SubTasks_Events, LCD_FREE, pdFALSE, pdTRUE,
				portMAX_DELAY);
		xQueueReceive(*UART_struct->UART_receive_Queue, &received_UART,
		...
		if (RETURN == charReceived) { /**if the received char is a return to menu*/
		...
		} else if (ENTER != charReceived) { /**if the character is anything different from enter and return chars,*/
...
void SPIReadHour(void * args) {
	...
	uint8_t printing_time_chars[12] = { '0' }; /**array that will hould the hour/date info to be printed on the spi screen*/
	uint8_t printing_date_chars[12] = { '0' }; /**array that will hould the hour/date info to be printed on the spi screen*/
	...
	{ /**task superloop*/
		...
		xEventGroupWaitBits(SubTasks_Events, LCD_FREE, pdTRUE, pdTRUE,
				portMAX_DELAY);
		...
		printing_date_chars[0] = ((time_buffer_variable[0] & TensMask) >> 4)
				+ '0';
		printing_date_chars[1] = (time_buffer_variable[0] & UnitsMask) + '0';
		printing_date_chars[2] = '/';
		printing_date_chars[3] = ((time_buffer_variable[1] & TensMask) >> 4)
				+ '0';
		printing_date_chars[4] = (time_buffer_variable[1] & UnitsMask) + '0';
		printing_date_chars[5] = '/';
		printing_date_chars[6] = '1';
		printing_date_chars[7] = (time_buffer_variable[0] & YearsMask >> 6)
				+ '0';
		printing_date_chars[8] = '\0';

		/**hour message zone indicating message is sent to the SPI screen*/
		uint8_t HourString[15] = { "Hour:       \0" };
		...
		printing_time_chars[9] = ' ';
		printing_time_chars[10] = ' ';
		printing_time_chars[11] = ' ';
		...
		timeString = pvPortMalloc(sizeof(SPI_msg_t));
		timeString->LCD_to_be_clear = pdFALSE;
		timeString->string = printing_date_chars;
		xQueueSend(*pSPI_queue, &timeString, portMAX_DELAY);
