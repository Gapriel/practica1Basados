...
static const uint8 ASCII[][5] = { { 0x00, 0x00, 0x00, 0x00, 0x00 } // 20
...
void LCDNokia_init(void)
{
    ...
    gpio_pin_config_t Nokia_pins_config = {kGPIO_DigitalOutput, 0};
    port_pin_config_t NokiaPinControlRegister = { kPORT_PullDisable,
    ...
    GPIO_PinInit(GPIOD, DATA_OR_CMD_PIN, &Nokia_pins_config);
    GPIO_PinInit(GPIOD, RESET_PIN, &Nokia_pins_config);
    PORT_SetPinConfig(PORTD, DATA_OR_CMD_PIN, &NokiaPinControlRegister);
    PORT_SetPinConfig(PORTD, RESET_PIN, &NokiaPinControlRegister);
...
uint8_t ScreenCleaningFlag = 0;
void LCDNokia_checkIfScreenFull(){
    CharCounter++;
    if(72 <=CharCounter){
        CharCounter = 0;
        ScreenCleaningFlag = 1;
    }
}
...
{
    uint16 index = 0;
    if(1 == ScreenCleaningFlag){
        ScreenCleaningFlag = 0;
        LCDNokia_clear();
    }
    LCDNokia_checkIfScreenFull();
